...
(() => {
  ...
  const stylesheet =
    ...
    // Compact empty state for small slots (e.g. avatar circles): hide the
    // "or browse files" sub-line and shrink the icon so nothing overflows.
    ':host{container-type:size}' +
    '@container (max-width: 130px) or (max-height: 130px){' +
    '  .empty{gap:2px;padding:6px}' +
    '  .empty .sub{display:none}' +
    '  .empty .cap{font-size:10.5px}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // Ignore unresolved template holes (e.g. "{{ photo }}") so no bogus
      // network fetch fires before the host framework resolves values.
